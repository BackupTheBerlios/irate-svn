...
public class MadEvent {
  ...
  public boolean hasStopped() {
    ...
    if(msg.length() <= i+("frames decoded".length())) 
      return false;
  ...
  // We just get a string from the libmad library, so we need to 
  // convert that into a time in seconds.
  public int getTimeSeconds() {
    String stringTime = getTime();
    int time = 0;
    if(stringTime != null ) {  
      // Seconds added
      time += Integer.parseInt(stringTime.substring(6));

      // Minutes added
      time += Integer.parseInt(stringTime.substring(3,5))*60;
      
      // Hour added
      time += Integer.parseInt(stringTime.substring(0,2))*216000;
    }
    return time;
  }
