...
void Template::loadOSDTemplate(const QString &file){
	...
	if(f.exists()){
		...
		QFile def(locate("html","en/kirateradio/default.osd"));
...
void Template::loadInfoTemplate(const QString &file){
	...
	if(f.exists()){
		...
		QFile def(locate("html","en/kirateradio/default.tmpl"));
...
QString Template::decodeText(const QString &text, SafeListViewItem* i){
	...
	TrackInfo* ti=i->getTrack();
	//Maybe this can be faster
	QString ret=text;
	ret=ret.replace("%SONG_RATE_ICON",this->keywords[ti->property("rateid","unrated")]).replace("%SONG_RATE_STRING",i->getStringRating()).replace("%SONG_LENGTH",i->lengthString());
