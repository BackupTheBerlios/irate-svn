  ...
  public void buildDialog() {
    ...
    // Create an 'Artist:' label
	  ...
	  gridData.widthHint = 80;
		...
		Image image = new Image(shell.getDisplay(), "images/magnify-icon.gif");
		...
		copyright.setText(currentTrack.getLicense().getFullText());
		copyright.pack();
		...
		if(currentTrack.getLicense().getIcon() != null && !currentTrack.getLicense().getIcon().equals("")) {	
			
			licenseButton = new Button(shell,SWT.FLAT);
			licenseButton.setImage(new Image(shell.getDisplay(), currentTrack.getLicense().getIcon()));
			gridData = new GridData();
	 ...
	 searchButton.addSelectionListener(new SelectionAdapter() {
		...
		if(licenseButton != null)
		{
			licenseButton.addSelectionListener(new SelectionAdapter() {
					public void widgetSelected(SelectionEvent e) {
						parentClient.showURL(currentTrack.getLicense().getURL().toString());
					}
				});
		} 
